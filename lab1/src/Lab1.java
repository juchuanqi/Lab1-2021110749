...
public class Lab1 {
    //D:\Graph\Graphviz\bin\dot.exe -Tpng -O graph.png D:\java0\lab1java\src\mygraph.dot
    // 图数据结构
    static class Graph {
    ...
    static String queryBridgeWords(Graph graph, String word1, String word2) {
        ...
        if (bridgeWords.isEmpty()) {
            ...
            if (bridgeWordsList.size() > 1) {
                ...
                return "The bridge words from " + word1 + " to " + word2 + " are: " + bridgeWordsList.get(0);
            }
        }
    }
    // 根据桥接词生成新文本
    ...
    static String calcShortestPath(Graph graph, String word1, String word2) {
        ...
        do {
            ...
            Map<String, Integer> currentWeights = graph.weights.get(current);
            if (currentWeights != null) {
                for (String neighbor : graph.adjList.get(current)) {
            ...
            current = distances.entrySet().stream()
                    ...
                    .min(Comparator.comparingInt(entry -> distances.getOrDefault(entry.getKey(), Integer.MAX_VALUE)))
                    .map(Map.Entry::getKey)
                    .orElse(null);
        } while (current != null && !current.equals(word2));
    
        if (current == null) {
            return "No path from \"" + word1 + "\" to \"" + word2 + "\"!";
        }
    
        List<String> path = computePath(previous, word2);
        return "Shortest path from \"" + word1 + "\" to \"" + word2 + "\": " + String.join(" --> ", path)
                + " (length: " + distances.get(word2) + ")";
    }
    
    private static List<String> computePath(Map<String, String> previous, String destination) {
